...
class Question extends Component {
  ...
  Onsubmit(e) {
    ...
    axios.post(`/Questions/questions`, Question)
      .then(res => {
        this.setState({ alert_msg: 'success', idd: res.data._id })
        axios.put(`/Lesson/addQTo/${this.state.l1}/${res.data._id}`).then(res => { console.log(res.data) });
      }).catch(error => {
  ...
  onsubmit1(e) {
    ...
    axios.post(`/Questions/questions`, Questionsingle)
      .then(res => {
        this.setState({ alert_msg: 'success', idd: res.data._id })
        axios.put(`/Lesson/addQTo/${this.state.l2}/${res.data._id}`).then(res => { console.log(res.data) });
      }).catch(error => {
        this.setState({ alert_msg: 'error' });
      })
  }
